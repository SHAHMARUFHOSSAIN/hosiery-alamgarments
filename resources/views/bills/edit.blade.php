...
@section('header', __('Edit Bill'))

@section('breadcrumb')
<nav aria-label="breadcrumb">
    <ol class="breadcrumb">
        <li class="breadcrumb-item"><a href="{{ route('dashboard') }}">{{ __('Dashboard') }}</a></li>
        <li class="breadcrumb-item"><a href="{{ route('bills.index', ['page' => $page]) }}">{{ __('Bills') }}</a></li>
...
<div class="card border-0 shadow-sm">
    <div class="card-body">
        <form method="POST" action="{{ route('bills.update', $bill) }}" enctype="multipart/form-data">
            ...
            <input type="hidden" name="bill_man" value="{{ request('bill_man') }}">
            <div class="row g-3">
                <div class="col-md-6">
                ...
                <div class="col-md-6">
                    ...
                    <select name="payment_type" id="payment_type" class="form-select @error('payment_type') is-invalid @enderror" required>
                        <option value="cash" {{ $currentType == 'cash' ? 'selected' : '' }}>{{ __('Cash') }}</option>
                        <option value="check" {{ $currentType == 'check' ? 'selected' : '' }}>{{ __('Cheque') }}</option>
                        <option value="tt" {{ $currentType == 'tt' ? 'selected' : '' }}>{{ __('TT') }}</option>
                        <option value="card" {{ $currentType == 'card' ? 'selected' : '' }}>{{ __('Reference Card') }}</option>
                        <option value="due" {{ $currentType == 'due' ? 'selected' : '' }}>{{ __('Due') }}</option>
                    </select>
                    @error('payment_type')
                    <div class="text-danger small">{{ $message }}</div>
                    @enderror
                    ...
                    <div id="checkPaymentsContainer">
                        ...
                        <div class="card border border-warning mb-3 check-payment-item" data-index="{{ $checkIndex }}">
                            <div class="card-header bg-warning text-dark py-2 d-flex justify-content-between align-items-center">
                                <span><i class="bi bi-bank"></i> {{ __('Cheque Payment') }} #{{ $loop->iteration }}</span>
                            ...
                            <div class="card-body">
                                <div class="row g-3">
                                    <div class="col-md-6" style="position: relative;">
                                        ...
                                        <div class="input-group">
                                            <input type="text" name="checks[{{ $checkIndex }}][bank_name]" class="form-control bank-search-input" placeholder="{{ __('Search bank name...') }}" autocomplete="off" value="{{ old('checks.' . $checkIndex . '.bank_name', $cp->bank_name) }}" required>
                                    ...
                                    <div class="col-md-4">
                                        ...
                                        <div class="mb-2">
                                            <a href="{{ route('cheque.show', $cp->check_photo) }}" target="_blank">
                                                 <img src="{{ route('cheque.show', $cp->check_photo) }}" alt="{{ __('Cheque') }}" class="img-thumbnail" style="width: 150px; height: 75px; object-fit: cover;">
                                            </a>
                                        </div>
                                        @else
                ...
                <div class="col-12" id="ttFields" style="{{ $currentType == 'tt' ? '' : 'display: none;' }}">
                    <div class="card border border-info">
                        ...
                        <div class="card-body">
                            <div class="row g-3">
                                <div class="col-md-6">
                                    <label for="tt_bank_name" class="form-label">{{ __('Bank Name') }} <span class="text-danger">*</span></label>
                ...
                <div class="col-12">
                    <button type="submit" class="btn btn-primary">
                        <i class="bi bi-save"></i> {{ __('Update Bill') }}
                    </button>
                </div>
            </div>
        </form>
