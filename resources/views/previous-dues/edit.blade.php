...
@section('title', __('Edit Previous Due'))
@section('header', __('Edit Previous Due'))
        ...
        <form method="POST" action="{{ route('previous-dues.update', $previousDue) }}">
            ...
            <div class="mb-3">
                ...
                <select name="status" class="form-select @error('status') is-invalid @enderror">
                    <option value="pending" {{ old('status', $previousDue->status) == 'pending' ? 'selected' : '' }}>{{ __('Pending') }}</option>
                    <option value="paid" {{ old('status', $previousDue->status) == 'paid' ? 'selected' : '' }}>{{ __('Paid') }}</option>
                </select>
            ...
            <div class="d-flex gap-2">
                ...
                <a href="{{ route('previous-dues.show', $previousDue) }}" class="btn btn-secondary">{{ __('Cancel') }}</a>
            </div>
        </form>
    </div>
</div>
